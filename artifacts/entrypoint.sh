...
OPTIONS="--nodiscover --networkid 42 --port 30303 --rpc --rpcport 8545 --rpcapi eth,net,web3,debug --etherbase 0x57e25703aba36bd2575e9027de2cb9ac187dc6da --verbosity 6"
...
case $1 in
	...
	./geth --rpccorsdomain "*" --verbosity "6" --identity $1 --rpcaddr $SUBNET.1 --mine --autodag --minerthreads "1" $OPTIONS
	;;
        node1)
	cp /root/key.node1 /root/.ethereum/nodekey
	sed -i "s/__subnet__/$SUBNET/g" /root/.ethereum/static-nodes.json
	./geth --rpccorsdomain "*" --verbosity "6" --identity $1 --rpcaddr $SUBNET.2 $OPTIONS
	;;
        node2)
	cp /root/key.node2 /root/.ethereum/nodekey
	sed -i "s/__subnet__/$SUBNET/g" /root/.ethereum/static-nodes.json
	./geth --rpccorsdomain "*" --verbosity "6" --identity $1 --rpcaddr $SUBNET.3 $OPTIONS
	;;
        ethstats)
